...
export default function LeadsTable({ onSelectLead }: LeadsTableProps) {
  ...
  if (isLoading) {
    return (
      <Card className="h-[500px] py-0 gap-0 flex flex-col border-none bg-[#FCFDFF] shadow-[0_15px_50px_-15px_rgba(0,0,0,0.03)] rounded-[3rem] overflow-hidden">
        <CardHeader className="p-8 px-10">
              ...
              >
                ...
                <TableCell className="px-10 py-6 text-center">
                  <span className="text-[9px] bg-primary/5 text-primary font-black px-3 py-1.5 rounded-xl ring-1 ring-primary/10 shadow-sm uppercase tracking-widest">
                    {new Date(lead.last_active).toLocaleDateString()}
                  </span>
                </TableCell>
                <TableCell className="px-10 py-6">
                  <div className="flex items-center gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8_rgba(16,185,129,0.5)] shrink-0" />
                    <span className="text-[11px] font-black text-foreground/60 tabular-nums uppercase">
                      {lead.stage}
                    </span>
                  </div>
                </TableCell>
